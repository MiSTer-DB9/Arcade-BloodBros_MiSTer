...
module BloodBros_tile_layer #(
	...
	wire [15:0] target_y = (vpos == V_VISIBLE - 9'd1) ? 16'd0
	                                                  : ({7'd0, vpos} + 16'd1);
	wire [15:0] eff_y_pf_w = target_y + scroll_y + {{6{yoff[9]}}, yoff};
	// Registriamo eff_y_pf per spezzare il path lungo
	//   vpos → add 16-bit → bit-slice → vram_addr write logic
	// (cluster dominante top-15: vpos[1..4] → vram_addr[8] del tile_layer).
	// 1 ciclo di latenza prima di PF_VRAM_R è invisibile (la FSM ha già
	// 3 stati di attesa BRAM PF_VRAM_R → W → W2 prima del rom fetch).
	reg [15:0] eff_y_pf;
	always @(posedge clk) eff_y_pf <= eff_y_pf_w;
	...
	// pen[3:0] CRUCIALE — mantenere ordering byte_lo prima, byte_hi dopo:
	wire [3:0] dec_pen     = {
	...
	wire        dec_dx_valid = (dec_dx >= 0) && (dec_dx < 256);
