...
module BloodBros_maincpu_map (
	...
	// ─── Region detect (MAME bloodbro.cpp:465-483) ──────────────────────────
	wire bus_active = ~bus_asn;
	wire is_rom     = bus_active && (bus_addr <  24'h080000);
	// Blood Bros: 44KB RAM (vs 48KB DCon), sprite RAM dentro RAM region!
	wire is_ram     = bus_active && (bus_addr >= 24'h080000) && (bus_addr < 24'h08B000);
	wire is_spr     = bus_active && (bus_addr >= 24'h08B000) && (bus_addr < 24'h08C000);
	wire is_bg      = bus_active && (bus_addr >= 24'h08C000) && (bus_addr < 24'h08C400);
	wire is_fg      = bus_active && (bus_addr >= 24'h08D000) && (bus_addr < 24'h08D400);
	wire is_mg      = 1'b0;   // Blood Bros: NO MG layer (solo BG/FG/Text)
	wire is_text    = bus_active && (bus_addr >= 24'h08D800) && (bus_addr < 24'h08E000);
	wire is_pal     = bus_active && (bus_addr >= 24'h08E800) && (bus_addr < 24'h08F800);
	wire is_gfxbank = 1'b0;   // Blood Bros: NO gfx_bank
	...
	wire is_bram    = is_ram | is_bg | is_fg | is_text | is_pal | is_spr;
