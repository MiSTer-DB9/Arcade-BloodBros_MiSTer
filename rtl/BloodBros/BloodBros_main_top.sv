...
// BloodBros_main_top — Top SD Gundam (1× M68000 @ 10 MHz, hardware Seibu D-Con).
//
// Sostituisce il top dual-68k Darius. Mantiene la STESSA interfaccia di
// darius_dual68k_top per non toccare BloodBros.sv:
...
//     audio già stubbato in BloodBros_audio_stub.sv)
//
...
(
	...
	input  wire [10:0] spr_vram_addr,    // Blood Bros: 2048 word sprite RAM
...
// Convenzione SD Gundam: scrollX += 128 nel dispatcher di disegno (lato BloodBros.sv).
...
// Sound comm bus: pilotato da Z80 audio reale (BloodBros_audio_z80).
...
// FG VRAM Blood Bros: bus_addr 0x8D000-0x8D3FF → bus_addr[11:1] = 0x000-0x1FF.
// Renderer FG legge BRAM[0..0x1FF]. NIENTE XOR (diverso da DCon dove FG era a
// 0x8C800 e serviva XOR 0x400).
BloodBros_vram_dp #(.AW(11)) u_fg_ram (
	.clk(clk), .a_we(fg_we), .a_be(map_vram_be),
	.a_addr(map_vram_addr), .a_din(map_vram_wdata), .a_dout(fg_rdata),
	.b_addr(fg_vram_addr), .b_dout(fg_vram_data)
);
BloodBros_vram_dp #(.AW(11)) u_mg_ram (
...
BloodBros_vram_dp #(.AW(11)) u_txt_ram (
...
// Sprite RAM CPU-side (write/read) — Blood Bros: 4KB = 2048 word (AW=11)
// MAME bloodbro.cpp:467 map(0x08b000, 0x08bfff).ram().share(m_spriteram) → 4KB.
BloodBros_vram_dp #(.AW(11)) u_spr_ram_cpu (
	.clk(clk), .a_we(spr_we), .a_be(map_vram_be),
	.a_addr(map_vram_addr[10:0]), .a_din(map_vram_wdata), .a_dout(spr_rdata),
	.b_addr(spr_copy_src_addr), .b_dout(spr_copy_src_data)
);

// Sprite RAM SHADOW — letto dallo sprite scanner durante render.
// Copy CPU→shadow al rising edge di vblank (replica DMA chip Seibu SEI0210).
// 2048 word total → counter 12-bit.
reg [11:0] spr_copy_cnt = 12'd2100;   // idle: > 2048 = done
reg        vblank_d = 1'b0;
...
module BloodBros_vram_word #(parameter AW = 11) (
	input  wire        clk,
...
module BloodBros_vram_dp #(parameter AW = 11) (
	input  wire           clk,
